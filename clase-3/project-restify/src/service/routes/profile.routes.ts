...
routerProfileInstance.get('/resolveById', async (req, res) => {
  try{
    const { profileId } = req.query // url?query=dato
    const profileElement = localData.find((el) => String(el.id) === String(profileId));
    if(!profileElement){
...
routerProfileInstance.get('/resolveByMultiplesIds', async (req, res) => {
  try{
    const { profileId } = req.query // url?query=dato
    const elementsFromSearch = localData.filter((el) => profileId.includes(String(el.id)));
    if(!elementsFromSearch.length){
     throw new Error('Elements does not exists');
    }
...
routerProfileInstance.post('/resolveByMultiplesIds', async (req, res) => {
  try{
    const { profileIds } = req.body // url?query=dato
    const elementsFromSearch = localData.filter((el) => profileIds.includes(String(el.id)));
    if(!elementsFromSearch.length){
     throw new Error('Elements does not exists');
    }
    return res.json(elementsFromSearch)
